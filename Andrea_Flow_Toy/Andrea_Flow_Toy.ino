...
LPD8806 strip = LPD8806(72, dataPin, clockPin);
...
//  wave(strip.Color(0,0,100), 4, 20);        // icy
...
void rainbowCycle(uint8_t wait) {
  ...
  for (j=0; j < 384 * 5; j++) {     // 5 cycles of all 384 colors in the wheel
    for (i=0; i < strip.numPixels(); i++) {
      ...
      setPixelColorWithIntensity(i, Wheel(((i * 384 / strip.numPixels()) + j) % 384), 80   );
    }
...
void setPixelColorWithIntensity(uint16_t pos, uint32_t c, uint8_t i) {
  byte g, r, b;
  g = (c >> 16) & 0x7f;
  r = (c >>  8) & 0x7f;
  b =  c        & 0x7f; 
  g = (float)g * ((float)i / 255.0);
  r = (float)r * ((float)i / 255.0);
  b = (float)b * ((float)i / 255.0);
...
uint32_t Wheel(uint16_t WheelPos, uint8_t intensity)
{
  float scale = 255.0 / (float)intensity;
  byte r, g, b;
  switch(WheelPos / 128)
  {
    case 0:
